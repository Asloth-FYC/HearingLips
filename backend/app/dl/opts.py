...
RAW_PATH = "/www/backend/app/dl/data"  #数据路径，要保证下一级文件夹是1.训练集和2.测试集

NUM_LABEL = "/www/backend/app/dl/label_map/num_label.txt"
NUM_LABEL_R = "/www/backend/app/dl/label_map/num_label_r.txt"
...
TEST_SUB = "/www/backend/app/dl/result/sub_2.csv"
num_class = 313  #类的数量
...
def file_deal(root_path, words_dict, list_file=TRAIN_LABEL):
    ...
    video_list = list(filter(lambda x: len(os.listdir(os.path.join(root_path, x))) >= 2, sig_names))  # 有效的文件名列表
...
parser.add_argument('--sub', default='sub', type=str, choices=['sub', 'prob'],
                    help='sub代表生成答案可提交，prob代表生成的答案有概率、用于模型投票融合')
...
parser.add_argument('--resume', default='E:\Download\dl.pth.tar', type=str, metavar='PATH',
