<template>
    <el-dialog
        ...
        <el-form :model="form" ref="form" size="medium" label-width="80px" :rules="rules">
            <el-form-item label="原密码" prop="Opsw" >
                <el-input placeholder="请输入原密码" type="password"
                          v-model="form.Opsw"></el-input>
            </el-form-item>
            <el-form-item label="密码" prop="psw" :error="errorText">
                <el-input placeholder="设置新密码" type="password" 
                          v-model="form.psw"></el-input>
            </el-form-item>
        ...
        methods:{
            onConfirm() {
                this.$refs.form.validate(valid => {
                    if (valid) {
                        if (this.form.psw !== this.form.confirmPassword) {
                            this.message({
                                message: '两次密码不一致',
                                type: 'warning'
                            });
